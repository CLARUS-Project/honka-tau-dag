...
def model_training_et(data: Dict[str, Any]):
    ...
    try:
        ...
        time.sleep(10)
        mlflow.set_experiment(config.MLFLOW_EXPERIMENT)
    with mlflow.start_run(run_name="ExtraTreesModelV2"):
        regr = ExtraTreesRegressor(max_depth=7,n_estimators=400)
        ...
        print(f"ExtraTrees model (depth={7:f}, n_estimator={400:f}):")
        print(f"  RMSE: {rmse}")
        print(f"  MAE: {mae}")
        print(f"  R2: {r2}")
        ...
        if tracking_url_type_store != "file":
            ...
            mlflow.sklearn.log_model(
                regr, "model", registered_model_name="ExtraTreesModelV2", signature=signature
            )
        else:
