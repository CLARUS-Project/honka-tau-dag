...
def model_training_vote(data: Dict[str, Any]):
    """
    Args:
    ...
    try:
        ...
        time.sleep(10)
        mlflow.set_experiment(config.MLFLOW_EXPERIMENT)
    with mlflow.start_run(run_name="VotingModel"):
        regr = VotingRegressor(estimators=[('dt', DecisionTreeRegressor()), ('ext', ExtraTreesRegressor())])
        ...
        print(f"Voting model (depth={7:f}, n_estimator={200:f}):")
        print(f"  RMSE: {rmse}")
        print(f"  MAE: {mae}")
        print(f"  R2: {r2}")
        ...
        if tracking_url_type_store != "file":
            ...
            mlflow.sklearn.log_model(
                regr, "model", registered_model_name="VotingModel", signature=signature
            )
        else:
